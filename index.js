...
function get20s(artistArray){
  const life20thCentury = [];
  for (let i = 0; i < artistArray.length; i++) {
    const birth = artistArray[i].years.slice(0, 4);
    const death = artistArray[i].years.slice(7);
    if ((birth >= 1900 && birth <= 2000) && (death >= 1900 && death <= 2000)) {
      life20thCentury.push(artistArray[i].name);
    }
  }
  return life20thCentury;
}
...
function lotsOfArt(artistArray){
  ...
  for (let i = 0; i < artistArray.length; i++) {
    if (artistArray[i].paintings > 100) {
      moreThan100Paintings.push(artistArray[i].name);
    }
  }
